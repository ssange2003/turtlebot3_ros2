...
static const TB3ModelInfo burger_info = {
  ...
  1, //
  0.033, // wheel_radius
  0.185, // wheel_separation 0.185 였나
  0.0925, // tuning_radius (wheel_separation / 2) 값의 근거
  0.250, // robot_radius (root(width^2+length^2))
  false,
};
...
void TurtleBot3Core::begin(const char* model_name)
{
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////
  motor_driver.getDxl().writeControlTableItem(ControlTableItem::OPERATING_MODE, 11, 3);
  motor_driver.getDxl().writeControlTableItem(TORQUE_ENABLE, 11, 1);
  motor_driver.getDxl().setGoalPosition(11, 2048);                                 // 초기 위치 이동
...
void TurtleBot3Core::run()
{
  ...
  if (millis()-pre_time_to_control_motor >= INTERVAL_MS_TO_CONTROL_MOTOR)
  {
    ...
    if(get_connection_state_with_motors() == true){
      ...
      // --- [✅ 이 2줄의 코드를 여기에 추가] ---
      int32_t current_goal_pos = controllers.getGoalPos11();
      motor_driver.getDxl().setGoalPosition(11, (float)current_goal_pos, UNIT_RAW);

      ///////////////////////////////////////////////////////////////////////////////////////////////////
      //motor_driver.write_goal_position(11, current_goal_pos);
  ///////////////////////////////////////////////////////////////////////////////////////////////////
    }
...
void update_analog_sensors(uint32_t interval_ms)
{
  ...
  if(millis() - pre_time >= interval_ms){
    ...
    //기존 코드: control_items.illumination = (uint16_t)sensors.getIlluminationData();
...
static void dxl_slave_write_callback_func(uint16_t item_addr, uint8_t &dxl_err_code, void* arg)
{
  ...
  switch(item_addr)
  {
    ...
    case ADDR_CMD_VEL_ANGULAR_Z:
      ...
      // --- [✅ 이 블록을 여기에 추가] ---
    case ADDR_CMD_VEL_LINEAR_Y: // 주소 154번
    {
        // 🛑 [필터링] 값이 '0' (Nav2의 기본값)이면 아무것도 하지 않고 무시!
        // control_items.cmd_vel_linear[1]은 정수형(int)이므로 0과 정확히 비교 가능
        if (control_items.cmd_vel_linear[1] != 0)

            //if문을 빼면 기존 코드랑 똑같다
        {
            // 0이 아닐 때만(사용자가 눌렀을 때만) 값을 변환하고 적용
        ...
        // 값이 0이면? -> 이 블록을 건너뜀 -> goal_pos_11 변수는 '이전 값'을 그대로 유지함.
        // 결과: 그리퍼가 현재 위치에서 버팀.
