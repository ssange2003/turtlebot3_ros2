...
bool Turtlebot3Controller::init(float max_lin_vel, float max_ang_vel, uint8_t scale_lin_vel, uint8_t scale_ang_vel)
{
  ...
  goal_pos_11 = 2048; // .h에서 선언한 변수 사용
...
void Turtlebot3Controller::getRCdata(float *get_cmd_vel)
{
  ...
  if (rc100_.available())
  {
    ...
    // [RC100이 있다면 이 코드는 ID 11 제어용으로 작동함]
    else if (received_data & RC100_BTN_2) // 2번 버튼: 값 증가
    {
        goal_pos_11 += 20;
    }
    else if (received_data & RC100_BTN_4) // 4번 버튼: 값 감소
    {
        goal_pos_11 -= 20;
    }
    ...
    // 모터 보호 로직
    if (goal_pos_11 > 4000) goal_pos_11 = 4000;
    if (goal_pos_11 < 100)  goal_pos_11 = 100;
