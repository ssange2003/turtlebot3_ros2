...
// ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
  ...
  const uint8_t JOINT_ID_ADD = 11;

  // 1. 운영 모드 (1-byte): 포인터 방식
  uint8_t op_mode = 5;
  // 'op_mode' 변수의 주소(&)를 1바이트(&op_mode) 길이(1)로 전달
  dxl.write(JOINT_ID_ADD, 11, &op_mode, 1);

  // [추가됨] 전류 제한(Current Limit) 설정: 주소 38번
  // 모드 5의 핵심입니다. 물체를 잡았을 때 300만큼의 힘으로만.
  uint16_t limit_val = 300;
  dxl.write(JOINT_ID_ADD, 38, (uint8_t*)&limit_val, 2);

  // 3. 토크 켜기
  uint8_t torque_enable = 1;
  dxl.write(JOINT_ID_ADD, 64, &torque_enable, 1);

  // 4. 초기 위치 잡기 (Goal Position)
  // 모드 5번도 위치 명령(116번)을 그대로 사용
  uint32_t goal_pos = 2048;
  dxl.write(JOINT_ID_ADD, 116, (uint8_t*)&goal_pos, 4);
...
bool Turtlebot3MotorDriver::write_goal_position(uint8_t id, int32_t goal_position)
{
    // [ ✅ 포인터 방식으로 수정 ]
    // 'goal_position' 변수의 주소(&)를 4바이트((uint8_t*)&goal_position) 길이(4)로 전달
    return dxl.write(id, 116, (uint8_t*)&goal_position, 4);
}
